...
wxBEGIN_EVENT_TABLE(SellObjectFrame, wxFrame)
    EVT_MENU(wxID_ABOUT, SellObjectFrame::OnAbout)
...
SellObjectFrame::SellObjectFrame(const wxString& title,const wxPoint& pos,const wxSize& size):wxFrame(NULL,wxID_ANY,title,pos,size){
    ...
    Bind(wxEVT_MENU, &SellObjectFrame::SearchButtonClick, this, searchMenuItem->GetId());
    ...
    wxStaticText* topic = new wxStaticText(TopicPanel,wxID_ANY,wxT("Sell Items"));
...
void SellObjectFrame::OnHello(wxCommandEvent& event)
{
    wxString msg;
...
void SellObjectFrame::OnAbout(wxCommandEvent& event)
{
    wxString msg;
...
void SellObjectFrame::OnQuit(wxCommandEvent& event)
{
    bool veto = Close();
}

void SellObjectFrame::onClose(wxCloseEvent& event){
    if(event.CanVeto()){
...
void SellObjectFrame::AddItem(wxPanel* panel,wxBoxSizer* panelSizer,MyScrolledWindow* parentPanel,wxBoxSizer* parentSizer){
    std::array<int, 4> id;
    id[0] = ids;
    ids++;
    id[1] = ids;
